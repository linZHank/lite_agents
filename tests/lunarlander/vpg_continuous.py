...
class PolicyNet(nnx.Module):
    ...
    def __init__(self, *, rngs: nnx.Rngs):
        self.linear1 = nnx.Linear(8, 128, rngs=rngs)  # 8-dim o_space
        self.linear2 = nnx.Linear(128, 128, rngs=rngs)
        self.linear3 = nnx.Linear(128, 2, rngs=rngs)  # 2-dim a_space
...
def objective_fn(actor: PolicyNet, experience_batch: ExperienceBatch):
    ...
    objective_value = (
        jnp.expand_dims(experience_batch.ret, axis=-1) * log_pi_as
    )  # expected return
...
plt.savefig(Path(__file__).parent.joinpath("vpg_continuous.png"))
