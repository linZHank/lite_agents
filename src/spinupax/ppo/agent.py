...
from spinupax.ppo.components import (
    PPOBuffer,
    ExperienceBatch,
...
@nnx.jit
def objective_fn(
    actor: nnx.Module, experience_batch: ExperienceBatch, clip_constant: float
):
...
def learn(
    ...
    ckpt_dir: PosixPath = Path("/tmp/spinupax/ppo/checkpoints/"),
    ...
    for e in range(max_epochs):
        for st in range((min_epoch_episodes + 1) * env.spec.max_episode_steps):
            # Play a step
            pred_act, last_val, old_logp = resolve_and_assess(
                rngs, actor, critic, last_obs
            )
            act = (
                pred_act.squeeze()
        ...
        for _ in range(actor_update_iters):
            obj_inv = update_actor_params(
                actor, actor_optimizer, experience_batch, clip_constant
            )
